...
export function getQueueStatus(): QueueConfig {
  const db = getDb();
  const config = db.prepare('SELECT * FROM queue_config WHERE id = 1').get() as QueueConfig | undefined;
...
export function setQueuePaused(paused: boolean): void {
  ...
  db.prepare('UPDATE queue_config SET state = ?, updated_at = ? WHERE id = 1').run(state, now);
}
...
export function reorderTasks(orders: TaskOrder[]): void {
  const db = getDb();
  const now = new Date().toISOString();

  const update = db.prepare('UPDATE tasks SET queue_order = ?, updated_at = ? WHERE id = ?');

  for (const item of orders) {
    update.run(item.order, now, item.id);
  }
}
